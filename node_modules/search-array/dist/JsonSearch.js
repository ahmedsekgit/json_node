...
var _QueryTokenizer = _interopRequireDefault(require("./QueryTokenizer.js"));
...
function filterQueries(query, searchKeys, searchKeysToObjectKeyMap) {
  const tokens = (0, _QueryTokenizer.default)(query, searchKeys);
  ...
  const containsKeyRegexs = tokens.filter(token => token.key !== undefined && !token.isNegate).map(token => {
    return {
      key: searchKeysToObjectKeyMap[token.key],
      regex: new RegExp(RegExpEscape(token.q), 'i')
    };
  });
  const notContainsKeyRegexs = tokens.filter(token => token.key !== undefined && token.isNegate).map(token => {
    return {
      key: searchKeysToObjectKeyMap[token.key],
      regex: new RegExp(RegExpEscape(token.q), 'i')
    };
...
class JsonSearch {
  ...
  filterFunc(query) {
    if (!query) return () => true; // search index keys
    ...
    return item => {
      ...
      let keyFound;
      let negatedKeywordNotFound = true; // here i have combined two separate for{} blocks for optimization purpose
