...
export default class Tokenizer {
    ...
    createLineCommentRegex(lineCommentTypes) {
        return new RegExp(`^((?:${lineCommentTypes.map(c => escapeRegExp(c)).join("|")}).*?(?:(\r)?\n|$))`);
    }
    ...
    getNextToken(input, previousToken) {
        return this.getWhitespaceToken(input) ||
            ...
            this.getOpenConditionToken(input) ||
            this.getConditionWordsToken(input) ||
            this.getCloseConditionToken(input) ||
    ...
    getOpenConditionToken(input) {
        return this.getTokenOnFirstMatch({
            input,
            type: tokenTypes.OPEN_CONDITION,
            regex: this.OPEN_CONDITION_REGEX
        });
    }

    getCloseConditionToken(input) {
        return this.getTokenOnFirstMatch({
            input,
            type: tokenTypes.CLOSE_CONDITION,
            regex: this.CLOSE_CONDITION_REGEX
        });
    }

    getConditionWordsToken(input) {
        return this.getTokenOnFirstMatch({
            input,
            type: tokenTypes.CONDITION_WORDS,
